...
<SvelteFlowProvider>
    ...
    {:else}
        ...
        <Pane
            ...
            collapsible={true}
            class="pt-14 fixed top-0 left-0 h-full z-20"
        >
            {#if $showControls}
                <div class="pr-4 pb-8 flex max-h-full min-h-full">
                    <div
                        class="w-full {($showOverview || $showArtifacts) && !$showCallOverlay
                            ? ' '
							: 'px-4 py-4 bg-white dark:shadow-lg dark:bg-gray-850 border dark:border-gray-850'}  rounded-xl z-40 pointer-events-auto overflow-y-auto scrollbar-hidden"
                    >
                        {#if $showCallOverlay}
